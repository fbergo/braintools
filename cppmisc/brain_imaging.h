...
public:
  ...
  void axisrot(const R3 &axis, float angle) {
    ...
    ryi.set(d, 0, -a, 0, 0, 1, 0, 0, a, 0, d, 0, 0, 0, 0, 1);

    rxi.set(1, 0, 0, 0, 0, c / d, -b / d, 0, 0, b / d, c / d, 0, 0, 0, 0, 1);
  ...
  void set(float c11, float c21, float c31, float c41, float c12, float c22,
           float c32, float c42, float c13, float c23, float c33, float c43,
  ...
  friend istream &operator>>(istream &s, T4 &t) {
    ...
    if (k.countTokens() != 19)
      goto t4inputerror;
    if (strcmp(k.nextToken(), "T4") != 0)
      goto t4inputerror;
    if (strcmp(k.nextToken(), "[") != 0)
      goto t4inputerror;
    for (i = 0; i < 4; i++)
      for (j = 0; j < 4; j++)
...
public:
  ...
  void sph2rect(R3 &dest, R3 &src) {
    ...
    t.X *= M_PI / 180.0;
    t.Y *= M_PI / 180.0;
    dest.set(cos(t.X) * sin(t.Y), sin(t.X) * sin(t.Y), cos(t.Y));
    dest.normalize();
    dest *= t.Z;
    dest += Center;
  }
  ...
  //! (dest.X,dest.Y,dest.Z) = (theta,phi,rho). Theta and phi are given in
  //! degrees.
...
public:
  ...
  void ycbcr2rgb() {
    ...
    g = 1.164 * ((float)R - 16.0) - 0.813 * ((float)B - 128.0) -
        0.392 * ((float)G - 128.0);
...
public:
  ...
  //! rectangle.
  void render(const char *text, int x, int y, Color &c, Paintable *p,
              float opacity = 1.0) {
    FT_GlyphSlot slot = face->glyph;
...
public:
  ...
  Image(char **xpm, Color &transp) { readXPM(xpm, transp); }
  ...
  //! given opacity
  void text(Font &f, const char *text, int x, int y, Color &c,
            float opacity = 1.0) {
  ...
  void floodFill(int x, int y, Color &c) {
    ...
    while (!fifo.empty()) {
      ...
      if (valid(px, py)) {
        ...
        if (valid(px - 1, py) && get(px - 1, py) == bg &&
            !done[(px - 1) + (py)*W]) {
          fifo.push((px - 1) + (py)*W);
          done[(px - 1) + (py)*W] = true;
        }
        ...
        if (valid(px, py - 1) && get(px, py - 1) == bg &&
            !done[(px) + (py - 1) * W]) {
          fifo.push((px) + (py - 1) * W);
          done[(px) + (py - 1) * W] = true;
        }
  ...
  //! the width and/or height of src are used.
  void blit(Image &src, int sx, int sy, int dx, int dy, int w = -1,
            int h = -1) {
  ...
  unsigned char *getBuffer() { return ((unsigned char *)data); }
...
public:
  ...
  bool operator<(const Location &b) const { return (sqlen() < b.sqlen()); }
...
public:
  ...
  Location &operator[](int index) { return (data[index]); }
...
public:
  ...
  int address(int x, int y, int z) { return (x + tby[y] + tbz[z]); }

  //! Returns the linear address of \ref Location loc
  int address(const Location &loc) { return (loc.X + tby[loc.Y] + tbz[loc.Z]); }
  ...
  bool valid(const Location &loc) { return (valid(loc.X, loc.Y, loc.Z)); }
...
public:
  ...
  //! Constructor
  BoundLocation(VolumeDomain &vd) : Location() { bind(vd); }

  //! Constructor
  BoundLocation(int w, int h, int d) : Location() { bind(w, h, d); }

  //! Copy constructor
  BoundLocation(BoundLocation &b) { (*this) = b; }
...
public:
  LocationIterator(int w, int h, int d) : Location() {
    W = w;
    H = h;
    D = d;
  }
  LocationIterator(VolumeDomain *vd) : Location() {
    W = vd->W;
    H = vd->H;
    D = vd->D;
  }
  ...
  LocationIterator &operator++(int a) {
    ++X;
    if (X == W) {
      X = 0;
      ++Y;
...
public:
  ...
  void set(int a, int b, int c, int d = 0) {
    x = a;
    y = b;
    z = c;
    count = (unsigned int)d;
  }
...
public:
  ...
  Volume(int w, int h, int d, float pdx = 1.0, float pdy = 1.0, float pdz = 1.0)
      : VolumeDomain(w, h, d) {
    int i;
  ...
  int writeSCN(const char *filename) {
    switch (sizeof(T)) {
    case 1:
      return (writeSCN(filename, 8, false));
    case 2:
      return (writeSCN(filename, 16, true));
    case 4:
      return (writeSCN(filename, 32, true));
    default:
      return -1;
    }
  }
  ...
  void readBzSCN(const char *filename) {
    ...
    case 8:
      ...
      for (i = 0; i < H * D; i++) {
        ...
        szPartial += W;
        if (bn != W || (berr != BZ_OK && berr != BZ_STREAM_END))
          goto bzrfail;
      }
      for (i = 0; i < N; i++)
        data[i] = (T)(td8[i]);
    ...
    case 16:
      ...
      for (i = 0; i < H * D; i++) {
        ...
        szPartial += 2 * W;
        if (bn != 2 * W || (berr != BZ_OK && berr != BZ_STREAM_END))
          goto bzrfail;
      }
      for (i = 0; i < N; i++)
        data[i] = (T)(td16[i]);
    ...
    case 32:
      ...
      for (i = 0; i < H * D; i++) {
        ...
        szPartial += 4 * W;
        if (bn != 4 * W || (berr != BZ_OK && berr != BZ_STREAM_END))
          goto bzrfail;
      }
      for (i = 0; i < N; i++)
        data[i] = (T)(td32[i]);
 ...
 */
  void readNIFTI(const char *filename) {
    ...
    if (*iptr != 348)
      return; // bad header
    if (strncmp(&hdr[344], "n+1\0", 4) != 0)
      return; // bad header
    sptr = (int16_t *)&hdr[40];
    nd = sptr[0];
    if (nd < 2 || nd > 3) {
      cerr << "** readNIFTI: only 2- and 3-dimensional images supported. "
              "Dimensions in "
           << filename << "=" << nd << endl;
      fclose(f);
      return;
    ...
    if (dtype == 32 || dtype == 128 || dtype == 1536 || dtype == 1792 ||
        dtype == 2048) {
      cerr << "** readNIFTI: file " << filename
           << " uses unsupported data type " << dtype << endl;
      fclose(f);
      return;
    }
    // sanity check
    if (W <= 0 || H <= 0 || D <= 0)
      goto nifti_read_fail;
    N = W * H * D;
    // printf("readNIFTI dim=%d %d %d dtype=%d pixdim=%.4f %.4f %.4f slope=%f
    ...
    case 2: { // uint8
      ...
      if (slope != 1.0f || intercept != 0.0f)
        for (i = 0; i < N; i++)
          t8u[i] = (uint8_t)(intercept + slope * t8u[i]);
      for (i = 0; i < N; i++)
        data[i] = (T)(t8u[i]);
      delete[] t8u;
    } break;
    case 256: { // int8
      ...
      if (slope != 1.0f || intercept != 0.0f)
        for (i = 0; i < N; i++)
          t8s[i] = (int8_t)(intercept + slope * t8s[i]);
      for (i = 0; i < N; i++)
        data[i] = (T)(t8s[i]);
      delete[] t8s;
    } break;
    case 4: { // int16
      ...
      if (slope != 1.0f || intercept != 0.0f)
        for (i = 0; i < N; i++)
          t16s[i] = (int16_t)(intercept + slope * t16s[i]);
      for (i = 0; i < N; i++)
        data[i] = (T)(t16s[i]);
      delete[] t16s;
    } break;
    case 512: { // uint16
      ...
      if (slope != 1.0f || intercept != 0.0f)
        for (i = 0; i < N; i++)
          t16u[i] = (uint16_t)(intercept + slope * t16u[i]);
      for (i = 0; i < N; i++)
        data[i] = (T)(t16u[i]);
      delete[] t16u;
    } break;
    case 8: { // int32
      ...
      if (slope != 1.0f || intercept != 0.0f)
        for (i = 0; i < N; i++)
          t32s[i] = (int32_t)(intercept + slope * t32s[i]);
      for (i = 0; i < N; i++)
        data[i] = (T)(t32s[i]);
      delete[] t32s;
    } break;
    case 768: { // uint32
      ...
      if (slope != 1.0f || intercept != 0.0f)
        for (i = 0; i < N; i++)
          t32u[i] = (uint32_t)(intercept + slope * t32u[i]);
      for (i = 0; i < N; i++)
        data[i] = (T)(t32u[i]);
      delete[] t32u;
    } break;
    case 1024: { // int64
      ...
      if (slope != 1.0f || intercept != 0.0f)
        for (i = 0; i < N; i++)
          t64s[i] = (int64_t)(intercept + slope * t64s[i]);
      for (i = 0; i < N; i++)
        data[i] = (T)(t64s[i]);
      delete[] t64s;
    } break;
    case 1280: { // uint64
      ...
      if (slope != 1.0f || intercept != 0.0f)
        for (i = 0; i < N; i++)
          t64u[i] = (uint64_t)(intercept + slope * t64u[i]);
      for (i = 0; i < N; i++)
        data[i] = (T)(t64u[i]);
      delete[] t64u;
    } break;
    case 16: { // float32
      ...
      if (slope != 1.0f || intercept != 0.0f)
        for (i = 0; i < N; i++)
          f32[i] = (float)(intercept + slope * f32[i]);
      for (i = 0; i < N; i++)
        data[i] = (T)(f32[i]);
      delete[] f32;
    } break;
    case 64: { // float64
      ...
      if (slope != 1.0f || intercept != 0.0f)
        for (i = 0; i < N; i++)
          f64[i] = (double)(intercept + slope * f64[i]);
      for (i = 0; i < N; i++)
        data[i] = (T)(f64[i]);
      delete[] f64;
    } break;
  ...
  T interpolatedVoxel(double x, double y, double z) {
    ...
    if (xx[0] < 0 || xx[0] >= W || yy[0] < 0 || yy[0] >= H || zz[0] < 0 ||
        zz[0] >= D)
      return 0;
    if (xx[1] < 0 || xx[1] >= W)
      xx[1] = xx[0];
    ...
    for (i = 0; i < 2; i++)
      for (j = 0; j < 2; j++)
        for (k = 0; k < 2; k++) {
          d = maxd -
              sqrt((xx[i] - x) * (xx[i] - x) + (yy[j] - y) * (yy[j] - y) +
                   (zz[k] - z) * (zz[k] - z));
  ...
  double stdev() { return (stdev(mean())); }
  ...
  Volume<int> *edt() {
    ...
    // cout << "initial state of 1,H-1,1 (this) = " << (int) (voxel(1,H-1,1)) <<
    // endl; cout << "initial state of 1,H-1,1 (edt)  = " << edt->voxel(1,H-1,1)
    // << endl;
  ...
  int computeOtsu() {
    ...
    for (t = 1; t < imax; t++) {
      for (i = 0, p1 = 0.0; i <= t; i++)
        p1 += hist[i];
      p2 = 1.0 - p1;
      if ((p1 > 0.0) && (p2 > 0.0)) {
        for (i = 0, m1 = 0.0; i <= t; i++)
          m1 += hist[i] * i;
        m1 /= p1;
        for (i = t + 1, m2 = 0.0; i <= imax; i++)
          m2 += hist[i] * i;
        m2 /= p2;
        for (i = 0, s1 = 0.0; i <= t; i++)
          s1 += hist[i] * (i - m1) * (i - m1);
        s1 /= p1;
        for (i = t + 1, s2 = 0.0; i <= imax; i++)
          s2 += hist[i] * (i - m2) * (i - m2);
        s2 /= p2;
  ...
  Volume<T> *interpolate(float ndx, float ndy, float ndz) {
      ...
      for (Q.X = 0; Q.X < tmp->W; Q.X++) {
        for (Q.Z = 0; Q.Z < tmp->D; Q.Z++)
          for (Q.Y = 0; Q.Y < tmp->H; Q.Y++) {
            ...
            value = (int)(((dx - dist_PQ) * (float)(dest->voxel(P)) +
                           dist_PQ * (float)dest->voxel(R)) /
                          dx);
            tmp->voxel(Q) = (T)value;
          }
      }
    ...
    if (ndy != dy) {
      tmp = new Volume<T>(dest->W, (int)(((float)dest->H - 1.0) * dy / ndy) + 1,
                          dest->D);
      for (Q.Y = 0; Q.Y < tmp->H; Q.Y++)
        for (Q.Z = 0; Q.Z < tmp->D; Q.Z++)
          for (Q.X = 0; Q.X < tmp->W; Q.X++) {
            ...
            value = (int)(((dy - dist_PQ) * (float)(dest->voxel(P)) +
                           dist_PQ * (float)(dest->voxel(R))) /
                          dy);
            tmp->voxel(Q) = (T)value;
          }
    ...
    if (ndz != dz) {
      tmp = new Volume<T>(dest->W, dest->H,
      ...
      for (Q.Z = 0; Q.Z < tmp->D; Q.Z++)
        for (Q.Y = 0; Q.Y < tmp->H; Q.Y++)
          for (Q.X = 0; Q.X < tmp->W; Q.X++) {
            ...
            value = (int)(((dz - dist_PQ) * (float)(dest->voxel(P)) +
                           dist_PQ * (float)(dest->voxel(R))) /
                          dz);
            tmp->voxel(Q) = (T)value;
          }
  ...
  Volume<T> *changeOrientation(Orientation from, Orientation to) {
...

    {
      ...
      if (fabs(ux.X) > 0.90)
        ndx = dx;
      else if (fabs(ux.Y) > 0.90)
        ndy = dx;
      else
        ndz = dx;
      if (fabs(uy.X) > 0.90)
        ndx = dy;
      else if (fabs(uy.Y) > 0.90)
        ndy = dy;
      else
        ndz = dy;
      if (fabs(uz.X) > 0.90)
        ndx = dz;
      else if (fabs(uz.Y) > 0.90)
        ndy = dz;
      else
        ndz = dz;
    }
  ...
  Volume<int> *edtTexture(Volume<int> *mr, Location &center,
                          Volume<int> **tmap = 0) {
    Volume<int> *tex;
    Sphere *S = 0;
  ...
  void planeFit(int z, Volume<char> *mask, T4 &X, int &niter,
                const char *outfile = NULL, bool verbose = false) {
    T4 lib[42], at, bestat;
    ...
    const char *desc[42] = {
        "TX/-1",   "TX/+1",   "TY/-1",   "TY/+1",   "TZ/-1",   "TZ/+1",
        "RX/-1",   "RX/+1",   "RY/-1",   "RY/+1",   "RZ/-1",   "RZ/+1",
        "TX/-5",   "TX/+5",   "TY/-5",   "TY/+5",   "TZ/-5",   "TZ/+5",
        "RX/-5",   "RX/+5",   "RY/-5",   "RY/+5",   "RZ/-5",   "RZ/+5",
        "RX/-10",  "RX/+10",  "RY/-10",  "RY/+10",  "RZ/-10",  "RZ/+10",
        "TX/-10",  "TX/+10",  "TY/-10",  "TY/+10",  "TZ/-10",  "TZ/+10",
        "RX/-0.5", "RX/+0.5", "RY/-0.5", "RY/+0.5", "RZ/-0.5", "RZ/+0.5",
    };
    ...
    do {
      ...
      if (whobest >= 0) {
        ...
        j++;
        if (verbose) {
          cout << "planeFit iteration " << j << ", " << desc[whobest]
               << ", score " << myscore << " => " << best << endl;
        }
    ...
    niter = j;
    if (verbose)
      cout << ">> " << j << " alignment iterations, optimization: " << fscore
           << " => " << best << endl;
  ...
  void transform(T4 &X) { transformNN(X); }
          ...
          for (iz = 0; iz < 2; iz++)
            for (iy = 0; iy < 2; iy++)
              for (ix = 0; ix < 2; ix++) {
                ...
                if (valid(jx, jy, jz)) {
                  w = sq3 -
                      sqrt((jx - a.X) * (jx - a.X) + (jy - a.Y) * (jy - a.Y) +
                           (jz - a.Z) * (jz - a.Z));
    ...
    for (k = 0; k < D; k++)
      for (j = 0; j < H; j++)
        for (i = 0; i < W; i++) {
          sx = sy = 0;
          for (p = 0; p < 6; p++) {
            if (valid(i + da[p], j + db[p], k))
              sx += w[p] * voxel(i + da[p], j + db[p], k);
            if (valid(i + db[p], j + da[p], k))
              sy += w[p] * voxel(i + db[p], j + da[p], k);
          }
  ...
  void render(Image &dest, T3 &rot, RenderingContext &rc, Color &objcolor,
              float opacity = 1.0, RenderType rendertype = RenderType_EQ,
              int rendervalue = 2) {
  ...
  void render(Image &dest, T3 &rot, RenderingContext &rc, Color &objcolor,
              R3 &lightvec, float opacity = 1.0,
              RenderType rendertype = RenderType_EQ, int rendervalue = 2,
              float kz = 1.50, float ka = 0.20, float ks = 0.40,
              float kd = 0.60) {
    float *zbuf, *nbuf, *xbuf, *ybuf, dw2, dh2;
    int *ibuf;
    char *rbuf;
  ...
  void edtRender(Image &dest, T3 &rot, RenderingContext &rc, int *lut,
                 ...
                 Volume<char> *tags = NULL, float tagopacity = 1.0,
...
private:
  ...
  void fft2d(float *data, int width, int height) {
    ...
    for (j = 0; j < H; j++)
      for (i = 0; i < W; i++)
        space(i, j) =
            (i < width && j < height) ? (double)(data[i + j * width]) : 0.0;
...
public:
  ...
  template <class T>
  void extractOblique(Volume<T> *src, R3 &grad, int x, int y, int z,
                      bool mirror = false) {
    R3 u, v, w;
  ...
  template <class T>
  void extractOblique(Volume<T> *src, Volume<int> *gradfield, int x, int y,
                      int z, bool mirror = false) {
    R3 g;
  ...
  double aratio(double th) {
    ...
    if (ca / cb < 1E-10 || ca / cb > 1E10)
      cout << "ca=" << ca << ", cb=" << cb << ", N=" << N
           << ", ca+cb=" << (ca + cb) << ", ca/cb=" << (ca / cb) << endl;
    return (ca / cb);
  }
  ...
  float mean() { return (sum() / N); }

  float stdev(const float vmean) {
    unsigned int i;
    float sum = 0.0;
    for (i = 0; i < N; i++)
      sum += (data[i] - vmean) * (data[i] - vmean);
    sum = sqrt(sum / N);
    return sum;
  }
...
public:
  MagPatch(int width, int height) : Patch(width, height) {
    ...
    if (fftw != width || ffth != height) {
      cerr << "(" << fftw << "," << ffth << ")\n";
      cerr << "** fatal: class MagPatch requires powers of two as dimensions. "
              "bye.\n\n";
      exit(77);
    }
...
private:
  ...
  int closest(float *fv, int *exc, int nexc) {
    ...
    for (i = 0; i < data.size(); i++) {
      if (binary_search(exc, exc + nexc, (int)i))
        continue;
      if (ibd < 0) {
        ibd = i;
