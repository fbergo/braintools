...
public:
  ...
  void drawSlider(int x, int y, int w, int h, float val, int sliderw = -1,
                  bool label = true) {
    int sx;
    char z[64];
  ...
  void repaint() { gtk_widget_queue_resize(widget); }
...
public:
  ...
  virtual void pressClient(int x, int y, int b) {
    ...
    if (b == 1 &&
        insideExtents(x, y, W - 20, sbvar[1], 15, sbvar[2] * msgs.size())) {
      gx = x;
      gy = y;
  ...
  virtual void releaseClient(int x, int y, int b) { gx = gy = -1; }
...
public:
  ...
  void clearViewRangeSync() { sync.clear(); }
...
public:
  ...
  void draw(GdkWindow *w, GdkGC *gc, int bx, int by, bool active, bool down) {
    ...
    gdk_draw_rectangle(w, gc, TRUE, bx + X + (down ? 3 : 0),
                       by + Y + (down ? 3 : 0), W, H);
...
public:
  ...
  virtual void paintClient() {
    ...
    for (i = 0; i < buttons.size(); i++)
      buttons[i]->draw(getWindow(), getGC(), X, Y + th, active[i],
                       ndown == (int)i);
  }
...
public:
  ...
  vector<string> composeHelp() {
    ...
    h = keyHelp();
    h.push_back(
        string("[Mouse] Left:Passive 2D Cursor, Right:Active 2D Cursor"));
    return (h);
  }

  virtual void invalidateViewRange() { invalidate(); }

  virtual void invalidateDepth() { invalidate(); }
  ...
  virtual void paintClient() {
    ...
    snprintf(z, 256, "Size = %d x %d x %d. (%d, %d, %d) = %d. Zoom %d %%",
             vol->W, vol->H, vol->D, cx, cy, cz, vol->voxel(cx, cy, cz),
             (int)(100.0 * zoom));
    ...
    if (rxy) {
      for (j = 0; j < vol->H; j++)
        for (i = 0; i < vol->W; i++) {
          ...
          if (seg != NULL && showseg) {
            if (seg->voxel(i, j, cz) == 1)
              c.mix(yellow, 0.25);
            else if (seg->voxel(i, j, cz) == 2)
              c.mix(yellow, 0.75);
          }
          if (edt != NULL) {
            v = edt->voxel(i, j, cz);
    ...
    if (rzy) {
      for (j = 0; j < vol->H; j++)
        for (i = 0; i < vol->D; i++) {
          ...
          if (seg != NULL && showseg) {
            if (seg->voxel(cx, j, i) == 1)
              c.mix(yellow, 0.25);
            else if (seg->voxel(cx, j, i) == 2)
              c.mix(yellow, 0.75);
          }
          if (edt != NULL) {
            v = edt->voxel(cx, j, i);
    ...
    if (rxz) {
      for (j = 0; j < vol->D; j++)
        for (i = 0; i < vol->W; i++) {
          ...
          if (seg != NULL && showseg) {
            if (seg->voxel(i, cy, j) == 1)
              c.mix(yellow, 0.25);
            else if (seg->voxel(i, cy, j) == 2)
              c.mix(yellow, 0.75);
          }
          if (edt != NULL) {
            v = edt->voxel(i, cy, j);
...
public:
  ...
  void setTags(Volume<char> *rt, Volume<char> *pt, Volume<int> *pm,
               Volume<int> *sqd) {
    rtag = rt;
    ptag = pt;
    pmap = pm;
  ...
  void pushRoundTag(int x, int y, int z, int bdiam, char label) {
    int n;
    if (ptag == NULL || pmap == NULL || rtag == NULL || rdepth == NULL)
      return;
  ...
  void flushRoundTags() {
    ...
    for (ui = 1; ui < pendingtags.size(); ui++) {
      if (minz(pendingtags[ui]) < z0)
        z0 = minz(pendingtags[ui]);
      if (maxz(pendingtags[ui]) > z1)
        z1 = maxz(pendingtags[ui]);
    }
    ...
    for (k = z0; k <= z1; k++)
      for (j = 0; j < ptag->H; j++)
        for (i = 0; i < ptag->W; i++)
          if (binary_search(pendingtags.begin(), pendingtags.end(),
                            pmap->voxel(i, j, k)))
  ...
  void roundTag(int px, int py, int pz, int x, int y, int z, int bdiam,
                char label) {
    int hx, hy, hz;
  ...
  void planarTag(int px, int py, int pz, int x, int y, int z, int bdiam,
                 char label) {
    int hx, hy, hz;
...
public:
  ...
  virtual void paintClient() {
    ...
    rect(X + 5, Y + th + fsy - 5, msw + 20 + 150, 16 * obj.size() + 10,
         ui.Border, false);
  }
  ...
  virtual void pressClient(int x, int y, int b) {
    ...
    if ((b == 1 || b == 3) &&
        insideExtents(x, y, 5, fsy - 5, fsx + 150, 16 * obj.size() + 10)) {
      int o;
      float ns;
      o = (y - fsy) / 16;
      ns = (x - fsx) / 100.0;
  ...
  virtual void releaseClient(int x, int y, int b) {
    if (insideExtents(x, y, 5, fsy - 5, fsx + 150, 16 * obj.size() + 10))
      return;
    releaseRotate(x, y, b);
  }
...
public:
  ...
  vector<string> composeHelp() {
    ...
    case 0:
      ...
      break;
    case 1:
      h.push_back(string("[Mouse] Left:Tag (Contour)"));
      break;
    }
    return (h);
  }
  ...
  virtual void paintClient() {
    ...
    if (orig != NULL) {
      ...
      drawSlider(dsx + 55, dsy - 1, 100, 16, ((float)getDepth()) / maxdepth, 4,
                 false);
  ...
  virtual void pressClient(int x, int y, int b) {
    if (dsx >= 0 && b == 1 &&
        insideExtents(x, y, lcx, lcy - 5, 220, 22 + 21 + 22)) {
      float nd;
      if (y > dsy - 5) {
...
private:
  ...
  void dragPoly(int x, int y, int b) { pressPoly(x, y, b); }
  ...
  void tagPolygon() {
    if (!poly.empty()) {
      ...
      for (i = 0; i < poly.size(); i++) {
        j = (i + 1) % poly.size();
        tmp->line(1 + (int)poly[i].X, 1 + (int)poly[i].Y, 1 + (int)poly[j].X,
                  1 + (int)poly[j].Y, c);
      }

      c = 0x000000;
      tmp->floodFill(0, 0, c);
      if (tmp->get(w - 1, h - 1) != 0)
        tmp->floodFill(w - 1, h - 1, c);
      if (tmp->get(0, h - 1) != 0)
        tmp->floodFill(0, h - 1, c);
      if (tmp->get(w - 1, 0) != 0)
        tmp->floodFill(w - 1, 0, c);
...
public:
  ...
  virtual void paintClient() {
    ...
    snprintf(z, 199, "M8: (%.2f / %.2f / %.2f) M16: (%.2f / %.2f / %.2f)", m[0],
             m[1], m[2], m[3], m[4], m[5]);
...
public:
  ...
  vector<string> composeHelp() {
    ...
    switch (getToolbarMode()) {
    case 0:
      h.push_back(string("[Mouse] Left:Feature Position"));
      break;
    case 1:
      h.push_back(string("[Mouse] Left:Tag (Contour)"));
      break;
  ...
  void setFV(FeatureView *fvp) { fv = fvp; }
  ...
  virtual void paintClient() {
    ...
    if (tex == NULL)
      return;
    if (PW != W || PH != H || unz == NULL || buf == NULL)
      invalid = 2;
    if (invalid)
      render();
  ...
  virtual void pressClient(int x, int y, int b) {
    ...
    if (unz == NULL || buf == NULL)
      return;

    if (b == 1 && getToolbarMode() == 1 &&
        insideExtents(x, y, 10, 10, buf->W, buf->H)) {
      R3 p;
  ...
  virtual void dragClient(int x, int y, int b) { pressClient(x, y, b); }
