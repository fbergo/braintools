...
int vy = 0, vz = 0, vx = 0;
int vmax = 1, nmax = 1;
float zoom = 1.0, uzoom = 1.0, ezoom = 1.0, axzoom = 1.0, paxzoom = 1.0, pipzoom = 1.0;
float opacity = 0.50;
int axpx=0, axpy=0;
int showfit = 1;
int showpip = 1;
...
public:
  ...
  ResliceTask(int f, int l) { first = f; last = l; }
  ResliceTask & operator=(const ResliceTask &a) { first = a.first; last = a.last; return(*this); }
  int first, last;
};
...
void draw_button(int x,int y,int w,int h,const char *label, int c1, int c2, int c3);
...
static GtkItemFactoryEntry ss_menu[] = {
  ...
  { "/Processing/Clear _Slice", NULL, GTK_SIGNAL_FUNC(ss_clear_slice), 0, NULL },
};
...
void ss_tool(int t) {
  ...
  case TOOL_PAN:
    ...
    break;
  case TOOL_RESLICE:
    status("Reslice tool: LEFT CLICK sagittal/coronal to set spine node, RIGHT CLICK sagittal/coronal to delete node, LEFT CLICK axial to navigate");
    break;
  case TOOL_AUTOSEG:
    status("Autoseg tool: LEFT CLICK to navigate, RIGHT CLICK axial to segment slice, CTRL+RIGHT CLICK to add multiple seeds. MID CLICK axial for zoom.");
    break;
  case TOOL_MANUALSEG:
    status("Manualseg tool: LEFT CLICK sagittal/coronal to navigate, LEFT CLICK axial to add points, RIGHT CLICK axial to remove points. MID CLICK axial for zoom.");
    break;
  case TOOL_RULER:
    status("Ruler tool: LEFT CLICK sagittal/coronal to navigate, LEFT CLICK axial to set points. MID CLICK axial for zoom.");
    break;
  }
  redraw(canvas);
}
...
void *ss_save_bg(void *arg) {
  ...
  *vio_total = -1;
  ...
  fprintf(f,"%d %d %d %d %d %d %d %d %d %d %d %d %d\n#spineseg state\n",
	  tool,vy,vz,(int) (100.0*uzoom),
	  pan[0], pan[1], nmax, (int) (100*opacity), 
	  showfit,showpip,vx,sagcor,stackcompose);
  fprintf(f,"#tool vy vz zoom panx pany nmax opacity showfit showpip vx sagcor stackcompose\n");
  fclose(f);
  ...
  for(i=0;i<rzn;i++)
    fprintf(f,"z %d %d\n",rzpz[i],rzpy[i]);
  if (withspline) fprintf(f,"spline\n");
  fclose(f);
  ...
  for(i=0;i<1024;i++)
    if (mline[i][0]>=0) {
      ...
      fprintf(f,"%d %d %d %d %d\n",i,mline[i][0],mline[i][1],mline[i][2],mline[i][3]);
    }
  fclose(f);
  ...
  fprintf(f,"0file,y,ecc,a/b,a(mm),b(mm),perim(mm),area(mm2),seg-area-min(mm2),seg-area-max(mm2),line-dist(mm)\n");

  scn_basename(base, volfile);
  bp = strrchr(base,'/'); if (bp==NULL) bp = base;
  ...
  for(i=0;i<vol->H;i++) {
	...
	for(j=0;j<rsd->W;j++) {
      ...
      fprintf(f,"%s,%d,%.4f,%.4f,%.2f,%.2f,%.2f,%.2f,%.1f,%.1f,%.1f\n",
	      ...
	      efit[i].perimeter() * rsd->dx,
	      efit[i].area() * rsd->dx * rsd->dz,
	      rsd->dx * rsd->dz * nv2,
	      rsd->dx * rsd->dz * nv,
	      mark_distance(i));
    }
...
void *ss_load_bg(void *arg) {
  ...
  *vio_total = -1;
  ...
  if (!rsd->ok()) {
    ...
    if (stackcompose) compute_stack_compose();
  } else {
    axialres = rsd->dx;
  }
  ...
  *vio_total = -1;
  ...
  if (f!=NULL) {
	...
	continue;
      ...
      if (strncmp(s,"spline",6)==0) withspline=1;
    }
    fclose(f);
    reslice_spline();
  } else {
  ...
  if (f!=NULL) {
    int val[13];
    fgets(s, 1024, f);
    if (sscanf(s,"%d %d %d %d %d %d %d %d %d %d %d %d %d",
	       &val[0],&val[1],&val[2],&val[3],
	       &val[4],&val[5],&val[6],&val[7],
	       &val[8],&val[9],&val[10],&val[11],&val[12]) == 13) {
      tool = val[0]; if (tool < TOOL_ZOOM && tool > TOOL_RULER) tool = 0;
      vy = val[1];   if (vy < 0 || vy >= vol->H) vy = vol->H/2;
      vz = val[2];   if (vz < 0 || vz >= vol->D) vz = vol->D/2;
      uzoom = val[3] / 100.0; if (uzoom < 0.10 || uzoom > 16.0) uzoom = 1.0;
      pan[0] = val[4];
      pan[1] = val[5];
      nmax = val[6]; if (nmax > vmax) nmax = vmax;
      opacity = val[7] / 100.0; if (opacity < 0.0 || opacity > 1.0) opacity = 0.50;
      showfit = val[8];
      showpip = val[9];
      vx = val[10];
      sagcor=val[11];
      stackcompose = val[12];
    } else if (sscanf(s,"%d %d %d %d %d %d %d %d %d %d %d %d",
	       &val[0],&val[1],&val[2],&val[3],
	       &val[4],&val[5],&val[6],&val[7],
      ...
      sagcor=val[11];
      stackcompose = 0;
    } else if (sscanf(s,"%d %d %d %d %d %d %d %d %d",
	       &val[0],&val[1],&val[2],&val[3],
	       &val[4],&val[5],&val[6],&val[7],
	       &val[8]) == 9) {
      tool = val[0]; if (tool < TOOL_ZOOM && tool > TOOL_RULER) tool = 0;
  ...
  if (f!=NULL) {
    int val[5];
    while(fgets(s, 1024, f)!=NULL) {
      if (sscanf(s,"%d %d %d %d %d",&val[0],&val[1],&val[2],&val[3],&val[4])==5) {
...
gboolean ls_del(GtkWidget *w, GdkEvent *e, gpointer data) {
  return FALSE;
}
...
void ls_ok(GtkWidget *w, gpointer data) {
  ...
  switch(o) {
  case 0: orient = Volume<int>::sagittal_lr; break;
  case 1: orient = Volume<int>::sagittal_rl; break;
  case 2: orient = Volume<int>::coronal_ap; break;
  case 3: orient = Volume<int>::coronal_pa; break;
  case 4: orient = Volume<int>::axial_fh; break;
  case 5: orient = Volume<int>::axial_hf; break;
  }

  stackcompose = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(lsd.compose)) ? 1 : 0;
...
void ls_cancel(GtkWidget *w, gpointer data) {
  gtk_widget_destroy(lsd.window);
}
...
void ss_load_special(GtkWidget *w, gpointer data) {
  ...
  lsd.orient = dropbox_new(",Sagittal (left to right),Sagittal (right to left),Coronal (anterior to posterior),Coronal (posterior to anterior),Transversal (bottom to top),Transversal (top to bottom)");
  ...
  lsd.compose = gtk_check_button_new_with_label("Compose axial slices by averaging the 3 nearest ones");
  ...
  gtk_signal_connect(GTK_OBJECT(ls), "delete_event",
		     GTK_SIGNAL_FUNC(ls_del), NULL);
  gtk_signal_connect(GTK_OBJECT(ls), "destroy",
		     GTK_SIGNAL_FUNC(ls_destroy), NULL);
  gtk_signal_connect(GTK_OBJECT(browse), "clicked",
		     GTK_SIGNAL_FUNC(ls_browse), NULL);
  gtk_signal_connect(GTK_OBJECT(ok), "clicked",
		     GTK_SIGNAL_FUNC(ls_ok), NULL);
  gtk_signal_connect(GTK_OBJECT(cancel), "clicked",
		     GTK_SIGNAL_FUNC(ls_cancel), NULL);
...
void draw_checkbox(int x, int y, int w, int h, const char *text, int checked) {
  ...
  if (checked)
    gdk_draw_rectangle(canvas->window,gc,TRUE,x+h/4,y+h/4,h/2+1,h/2+1);
...
gboolean ss_expose(GtkWidget *w, GdkEventExpose *e, gpointer data) {
  ...
  if (vol==NULL && vio_loading==0) return TRUE;
  ...
  if (vol!=NULL && vio_loading==0) {
    ...
    for(i=0;i<rsn;i++) {
      ...
      dst->rect( (int)((rspx[i] * dstzoom) - 2), (int)((rspy[i]*dstzoom) - 2), 5, 5, pink, false);

      if ((rsn < 3 || !withspline) && (i < rsn - 1))
	dst->line( (int)(rspx[i] * dstzoom) , 
		   (int)(rspy[i] * dstzoom) , 
		   (int)(rspx[i+1] * dstzoom) , 
		   (int)(rspy[i+1] * dstzoom), pink);
	...
	dst->line( (int) (splinex[i] * dstzoom),
		   (int) (i * dstzoom),
		   (int) (splinex[i+1] * dstzoom),
		   (int) ((i+1) * dstzoom), pink );
    }
...

    {
      ...
      dst->line( (int) (px * dstzoom) - 10, 
		 (int) (vy * dstzoom), 
		 (int) (px * dstzoom) + 10,
		 (int) (vy * dstzoom), lgreen); 
      
      dst->line( (int) (px * dstzoom), 
		 (int) (vy * dstzoom) - 10, 
		 (int) (px * dstzoom),
		 (int) (vy * dstzoom) + 10, lgreen); 
    ...
    for(i=0;i<rzn;i++) {
      ...
      dst->rect( (int)((rzpz[i] * dstzoom) - 2), (int)((rzpy[i]*dstzoom) - 2), 5, 5, pink, false);

      if ((rzn < 3 || !withspline) && (i < rzn - 1))
	dst->line( (int)(rzpz[i] * dstzoom) , 
		   (int)(rzpy[i] * dstzoom) , 
		   (int)(rzpz[i+1] * dstzoom) , 
		   (int)(rzpy[i+1] * dstzoom), pink);
	...
	dst->line( (int) (splinez[i] * dstzoom),
		   (int) (i * dstzoom),
		   (int) (splinez[i+1] * dstzoom),
		   (int) ((i+1) * dstzoom), pink );
    }
    ...
    // draw reslice plane
    {
      ...
      dst->line( (int) (pz * dstzoom) - 10, 
		 (int) (vy * dstzoom), 
		 (int) (pz * dstzoom) + 10,
		 (int) (vy * dstzoom), lgreen); 
      
      dst->line( (int) (pz * dstzoom), 
		 (int) (vy * dstzoom) - 10, 
		 (int) (pz * dstzoom),
		 (int) (vy * dstzoom) + 10, lgreen); 
    ...
    panes[0].x = 10 + pan[0]; panes[0].y = 10 + pan[1]; panes[0].w = tmp->W; panes[0].h = tmp->H;
    ...
    if (showpip) {
      ...
      gdk_draw_rectangle(w->window,gc,FALSE,panes[6].x,panes[6].y,panes[6].w,panes[6].h);
    }
	...
	if (i==evz || j==evx) c.mix(green, 0.33);
	if (seg->voxel(j,vy,i) == 1) c.mix(yellow, opacity);
	if (seg->voxel(j,vy,i) == 2) c.mix(orange, opacity);
	if (seed->voxel(j,vy,i) != 0) c.mix(red, 1.0);
	if (rsdirty) { c.G /= 4; c.B /= 4; }
	axi->set(i,j,c);
      }
	...
	for(i=0;i<100;i++) {
	  ...
	  x1 = panes[1].x + axpx + (int) (0.5+eplot.e((i+1)%100,0) * axzoom);
	  y1 = panes[1].y + axpy + (int) (0.5+eplot.e((i+1)%100,1) * axzoom);
    ...
    if (zmode > 0)
      left_label(panes[1].x+10,panes[1].y,panes[1].w,20,zms[zmode],0xffff00,0);

    gdk_gc_set_clip_rectangle(gc,NULL);
    
    if (rsdirty)
      center_label(panes[1].x,panes[1].y,panes[1].w,panes[1].h,
		   "Invalid View: Reslicing Required",
		   0xffff00,1);
    ...
    // toolbar
    gdk_draw_drawable(w->window, gc, toolsp[ tool == TOOL_ZOOM      ?  1 :  0 ], 0,0, 5,5+34*0,32,32);
    gdk_draw_drawable(w->window, gc, toolsp[ tool == TOOL_PAN       ?  3 :  2 ], 0,0, 5,5+34*1,32,32);
    gdk_draw_drawable(w->window, gc, toolsp[ tool == TOOL_RESLICE   ?  5 :  4 ], 0,0, 5,5+34*2,32,32);
    gdk_draw_drawable(w->window, gc, toolsp[ tool == TOOL_AUTOSEG   ?  7 :  6 ], 0,0, 5,5+34*3,32,32);
    gdk_draw_drawable(w->window, gc, toolsp[ tool == TOOL_MANUALSEG ?  9 :  8 ], 0,0, 5,5+34*4,32,32);
    gdk_draw_drawable(w->window, gc, toolsp[ tool == TOOL_RULER     ? 11 : 10 ], 0,0, 5,5+34*5,32,32);
    ...
    if (tool == TOOL_RESLICE) {
      ...
      center_label(42,5+34*2,64,32, withspline ? "Spline" : "Linear", 0, 0);
	...
	for(i=0;i<rsd->W;i++)
	  for(j=0;j<rsd->D;j++) {
	    if (seg->voxel(i,vy,j)!=0) nv++;
	    if (seg->voxel(i,vy,j)==1) nv2++;
	  }
	...
	sprintf(msg,"Ellipse properties: ecc=%.4f a/b=%.4f a=%.2f mm b=%.2f mm perimeter=%.1f mm area=%.1f mm² (seg=%.1f-%.1f mm²) line=%.1f mm",
		...
		efit[vy].perimeter() * rsd->dx,
		efit[vy].area() * rsd->dx * rsd->dz,
		rsd->dx * rsd->dz * nv2,
		rsd->dx * rsd->dz * nv,
		mark_distance(vy));
    ...
    // viewing window
    panes[2].x = W-110; panes[2].y = H-80; panes[2].w = 100; panes[2].h = 20;
    gc_color(gc,0);
    gdk_draw_rectangle(w->window,gc,TRUE,panes[2].x,panes[2].y,panes[2].w,panes[2].h);
    
    j = (100*nmax) / vmax;
    gc_color(gc,0xc0c080);
    gdk_draw_rectangle(w->window,gc,TRUE,panes[2].x,panes[2].y,j,panes[2].h);
    
    gc_color(gc,0xffffff);
    gdk_draw_rectangle(w->window,gc,FALSE,panes[2].x,panes[2].y,panes[2].w,panes[2].h);
    center_label(panes[2].x,panes[2].y,panes[2].w,panes[2].h,"Window",0xffffff, 1);
    
    // segmentation opacity
    panes[3].x = W-110; panes[3].y = H-50; panes[3].w = 100; panes[3].h = 20;
    gc_color(gc,0);
    gdk_draw_rectangle(w->window,gc,TRUE,panes[3].x,panes[3].y,panes[3].w,panes[3].h);
    
    j = (int) (100*opacity);
    gc_color(gc,0xc0c080);
    gdk_draw_rectangle(w->window,gc,TRUE,panes[3].x,panes[3].y,j,panes[3].h);
    
    gc_color(gc,0xffffff);
    gdk_draw_rectangle(w->window,gc,FALSE,panes[3].x,panes[3].y,panes[3].w,panes[3].h);
    center_label(panes[3].x,panes[3].y,panes[3].w,panes[3].h,"Opacity",0xffffff, 1);
    
    // show fit
    panes[4].x = W-210; panes[4].y = H-50; panes[4].w = 100; panes[4].h = 20;
    draw_checkbox(panes[4].x, panes[4].y, panes[4].w, panes[4].h, "Show fit", showfit);
    
    // show pip
    panes[5].x = W-310; panes[5].y = H-50; panes[5].w = 100; panes[5].h = 20;
    draw_checkbox(panes[5].x, panes[5].y, panes[5].w, panes[5].h, "Show PIP", showpip);
...
gboolean ss_press(GtkWidget *w, GdkEventButton *e, gpointer data) {
  ...
  if (vio_active) return TRUE;
  if (rsqactive) return TRUE;
  ...
  if (vol!=NULL) {
    // tool selection
    if (b==1 && inbox(x,y,5,5,32,32))     { ss_tool(0); return TRUE; }
    if (b==1 && inbox(x,y,5,5+34,32,32))  { ss_tool(1); return TRUE; }
    if (b==1 && inbox(x,y,5,5+68,32,32))  { ss_tool(2); return TRUE; }
    if (b==1 && inbox(x,y,5,5+102,32,32)) { ss_tool(3); return TRUE; }
    if (b==1 && inbox(x,y,5,5+136,32,32)) { ss_tool(4); return TRUE; }
    if (b==1 && inbox(x,y,5,5+170,32,32)) { ss_tool(5); return TRUE; }
	...
	if (uzoom < 0.10) uzoom = 0.10;
	if (uzoom > 16.0) uzoom = 16.0;
	zs[3] = x;
      }
      redraw(canvas);
      return TRUE;
    ...
    if (b==1 && inbox(x,y,panes[0].x,panes[0].y,panes[0].w,panes[0].h)) {
      ...
      case TOOL_RESLICE:
	if (!sagcor)
	  reslice_set_sag_node( (int)((x-panes[0].x) / ezoom), (int)((y-panes[0].y) / ezoom) );
	else
	  reslice_set_cor_node( (int)((x-panes[0].x) / ezoom), (int)((y-panes[0].y) / ezoom) );
	redraw(canvas);
	return TRUE;
      default:
    ...
    if (b==3 && inbox(x,y,panes[0].x,panes[0].y,panes[0].w,panes[0].h) && tool==TOOL_RESLICE) {
      if (!sagcor)
	...
	break;
      case TOOL_MANUALSEG:
	ss_manualseg((int) ((y - panes[1].y - axpy) / axzoom),
		     vy,
		     (int) ((x - panes[1].x - axpx) / axzoom),
		     1);
	redraw(canvas);
	break;
      case TOOL_RULER:
...
gboolean ss_release(GtkWidget *w, GdkEventButton *e, gpointer data) {
  ...
  if (tool == TOOL_ZOOM) { zs[0] = 0; redraw(canvas); }
  if (tool == TOOL_PAN)  { ps[0] = 0; redraw(canvas); }
...
gboolean ss_key(GtkWidget *w, GdkEventKey *e,gpointer data) {
  ...
  if (vio_active) return TRUE;
  if ( (e->state & GDK_MOD1_MASK) != 0) return FALSE; // ignore Alt+anything

  switch(e->keyval) {
  case GDK_Left:
    if (vol!=NULL) { 
      if (!sagcor) { vz--; if (vz<0) vz=0; }
      else { vx--; if (vx<0) vx=0; }
      redraw(canvas); 
    }
    break;
  case GDK_Right:
    if (vol!=NULL) { 
      if (!sagcor) { vz++; if (vz>=vol->D) vz=vol->D-1; }
      else { vx++; if (vx>=vol->W) vx=vol->W+1; }
      redraw(canvas); 
    }
    break;
  case GDK_Up:
    if (vol!=NULL) { vy--; if (vy<0) vy=0; redraw(canvas); }
    break;
  case GDK_Down:
    if (vol!=NULL) { vy++; if (vy>=vol->H) vy=vol->H-1; redraw(canvas); }
    break;
  case GDK_Escape:
    ss_clear(NULL,NULL);
    break;
  ...
  case GDK_S:
  case GDK_s:
    if (opacity < 0.1) opacity = 0.50; else opacity = 0.0;
    redraw(canvas);
    break;
  case GDK_z:
  case GDK_Z:
...
void gui() {
  ...
  toolsp[0] = gdk_pixmap_create_from_xpm_d(mw->window, &toolsm[0], &style->bg[GTK_STATE_NORMAL], 
					   (gchar **) zoom0_xpm);
  toolsp[1] = gdk_pixmap_create_from_xpm_d(mw->window, &toolsm[0], &style->bg[GTK_STATE_NORMAL], 
					   (gchar **) zoom1_xpm);
  toolsp[2] = gdk_pixmap_create_from_xpm_d(mw->window, &toolsm[0], &style->bg[GTK_STATE_NORMAL], 
					   (gchar **) pan0_xpm);
  toolsp[3] = gdk_pixmap_create_from_xpm_d(mw->window, &toolsm[0], &style->bg[GTK_STATE_NORMAL], 
					   (gchar **) pan1_xpm);
  toolsp[4] = gdk_pixmap_create_from_xpm_d(mw->window, &toolsm[0], &style->bg[GTK_STATE_NORMAL], 
					   (gchar **) normal0_xpm);
  toolsp[5] = gdk_pixmap_create_from_xpm_d(mw->window, &toolsm[0], &style->bg[GTK_STATE_NORMAL], 
					   (gchar **) normal1_xpm);
  toolsp[6] = gdk_pixmap_create_from_xpm_d(mw->window, &toolsm[0], &style->bg[GTK_STATE_NORMAL], 
					   (gchar **) auto0_xpm);
  toolsp[7] = gdk_pixmap_create_from_xpm_d(mw->window, &toolsm[0], &style->bg[GTK_STATE_NORMAL], 
					   (gchar **) auto1_xpm);
  toolsp[8] = gdk_pixmap_create_from_xpm_d(mw->window, &toolsm[0], &style->bg[GTK_STATE_NORMAL], 
					   (gchar **) manual0_xpm);
  toolsp[9] = gdk_pixmap_create_from_xpm_d(mw->window, &toolsm[0], &style->bg[GTK_STATE_NORMAL], 
					   (gchar **) manual1_xpm);
  toolsp[10] = gdk_pixmap_create_from_xpm_d(mw->window, &toolsm[0], &style->bg[GTK_STATE_NORMAL], 
					    (gchar **) dist0_xpm);
  toolsp[11] = gdk_pixmap_create_from_xpm_d(mw->window, &toolsm[0], &style->bg[GTK_STATE_NORMAL], 
					    (gchar **) dist1_xpm);
  ...
  gtk_signal_connect(GTK_OBJECT(mw),"delete_event",GTK_SIGNAL_FUNC(ss_del),NULL);
  gtk_signal_connect(GTK_OBJECT(mw),"destroy",GTK_SIGNAL_FUNC(ss_destroy),NULL);
  gtk_signal_connect(GTK_OBJECT(canvas),"expose_event",GTK_SIGNAL_FUNC(ss_expose),NULL);
  gtk_signal_connect(GTK_OBJECT(canvas),"button_press_event",GTK_SIGNAL_FUNC(ss_press),NULL);
  gtk_signal_connect(GTK_OBJECT(canvas),"button_release_event",GTK_SIGNAL_FUNC(ss_release),NULL);
  gtk_signal_connect(GTK_OBJECT(canvas),"motion_notify_event",GTK_SIGNAL_FUNC(ss_drag),NULL);
  gtk_signal_connect(GTK_OBJECT(mw),"key_press_event",GTK_SIGNAL_FUNC(ss_key),NULL);
...
int main(int argc, char **argv) {
  ...
  for(i=1;i<argc;i++) {
    ...
    if (!strcmp(argv[i],"-c")) {
      stackcompose = 1;
      printf("each axial slice will be composed of the 3 nearest ones by averaging.\n");
      continue;
    }
    if (!strcmp(argv[i],"-h") || (!strcmp(argv[i],"--help"))) {
      printf("SpineSeg version %s - (c) 2009-2017 Felipe Bergo <fbergo\x40gmail.com>\n\n",VERSION);
      printf("Command line options:\n");
      printf(" -debug     enable debugging actions.\n");
      printf(" -h         show this help message and exit.\n");
      printf(" -r value   override axial interpolation resolution (default 0.50 mm)\n");
      printf(" -c         compose each axial slice from the nearest 3\n");
      printf(" -g value   override morphological gradient radius (default 1.0 pixels)\n\n");
      exit(0);
    }
    ...
    if (!strcmp(argv[i],"-g") && i<argc-1) {
      ...
      if (GradRadius < 1.0) GradRadius = 1.0;
      if (GradRadius > 10.0) GradRadius = 10.0;
...
double mark_distance(int y) {
  if (y<0 || y>=1024) return 0.0;
  if (vol==NULL) return 0.0;
  if (mline[y][0] < 0 || mline[y][2] < 0) return 0.0;
  return(rsd->dx * hypot(mline[y][0]-mline[y][2],mline[y][1]-mline[y][3]));
}
...
void reslice_compute_all() {
  if (rsn > 0 || rzn > 0)
    reslice_compute_interval(Min(rspy[0],rzpy[0]),Max(rspy[rsn-1],rzpy[rzn-1]));
}
...
void _reslice_compute_single(int y) {
  ...
  for(j=0;j<rsd->D;j++)
	...
	sv = 0.0;
	for(t=0;t<8;t++) {
	  w = 1.0 / sqrt( (cx[t]-q.X)*(cx[t]-q.X)+(cy[t]-q.Y)*(cy[t]-q.Y)+(cz[t]-q.Z)*(cz[t]-q.Z) );
	  if (w > 20.0) w = 20.0;
	  sw += w;
...
void compute_stack_compose() {
  ...
  for(j=1;j<tmp->H-1;j++)
    for(k=0;k<tmp->D;k++)
      for(i=0;i<tmp->W;i++)
	tmp->voxel(i,j,k) = (rsd->voxel(i,j,k) + 
			     rsd->voxel(i,j-1,k) + 
			     rsd->voxel(i,j+1,k) ) / 3;
