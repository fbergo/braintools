...
public:
  ...
  T &e(int r, int c) { return (value[r * cols + c]); }

  // math element (index start at 1)
  T &me(int r, int c) { return (value[(r - 1) * cols + (c - 1)]); }

  // element (index start at 0)
  T ce(int r, int c) const { return (value[r * cols + c]); }

  // math element (index start at 1)
  T cme(int r, int c) const { return (value[(r - 1) * cols + (c - 1)]); }
    ...
    for (i = 1; i <= 50; i++) {
      ...
      for (ip = 1; ip <= rows - 1; ip++) {
        for (iq = ip + 1; iq <= rows; iq++) {
          ...
          if (i > 4 && fabs(eval.me(ip, 1)) + g == fabs(eval.me(ip, 1)) &&
            ...
            me(ip, iq) = 0;
            for (j = 1; j <= ip - 1; j++)
              jacobi_rotate(j, ip, j, iq, tau, s);
            for (j = ip + 1; j <= iq - 1; j++)
              jacobi_rotate(ip, j, j, iq, tau, s);
            for (j = iq + 1; j <= rows; j++)
              jacobi_rotate(ip, j, iq, j, tau, s);
            for (j = 1; j <= rows; j++)
              evec.jacobi_rotate(j, ip, j, iq, tau, s);
            ++nrot;
          }
        }
      }
...
public:
  ...
  double flatness() { return (major_semiaxis() / minor_semiaxis()); }
  ...
  void fit() {
    ...
    // fetch the solution (eigenvector associated to the single negative
    // eigenvalue)
    for (j = 1; j <= 6; j++)
